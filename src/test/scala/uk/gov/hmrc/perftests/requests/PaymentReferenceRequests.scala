...
import uk.gov.hmrc.perftests.requests.AuthLoginRequests.redirectUrl

object PaymentReferenceRequests extends ServicesConfiguration with RequestUtils {
  ...
  def enterPaymentRefNumber(paymentRefNumber: String): HttpRequestBuilder =
    http("Enter Payment Reference number")
      ...
      .formParam("value", paymentRefNumber)
      .check(status.is(303))
}
